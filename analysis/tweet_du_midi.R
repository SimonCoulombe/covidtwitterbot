...
pct_positif <- hist %>% filter(Nom=="Ensemble du Québec", !is.na(psi_quo_pos_t)) %>% filter(date== max(date)) %>% pull(psi_quo_pos_t)

## https://unicode.org/emoji/charts/full-emoji-list.html

post_tweet(
  status = paste0(intToUtf8(0x1F9F5), "covid du ",format(as.Date(Sys.Date()), format = format_francais) ,"\n\n",
                  "Sommaire:\n",
...
post_tweet(
  status = paste0(intToUtf8(0x1F4C8), " Tests, hospitalisations, soins intensifs et décès de #covid\n",
