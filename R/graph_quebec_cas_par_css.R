...
carte_css <- function(css_last_week = NULL, type = "maximum") {
  ...
  g <- ggplot(data = css_last_week) +
    ...
    } +
    {
      if (type == "maximum500rouge") {
        scale_fill_gradientn(
          colours = c("white", palette_OkabeIto["vermillion"]),
    ...
    labs(
      title = paste0("Nouveaux cas de covid par million d'habitants par centre de services scolaires"),
      ...
      caption = paste0(
        "Les données sont publiées au niveau des réseaux locaux de service (RLS) et distribuées aux centres de services scolaires (CS)",
