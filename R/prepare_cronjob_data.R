...
import_unique_csvs_from_cronjob <- function(mypath = "~/cronjob/tableau-rls/",
  ...
  if (mypath =="~/cronjob/inspq-tableau-rls-new/"){
    ...
    csvs <- purrr::map(
      pouet, read_delim, skip = skip, n_max = n_max, locale = locale, delim = delim,
      col_types= cols(No = col_character(),
                      RSS = col_character() ,
                      NoRLS = col_character(),
                      RLS= col_character(),
                      .default = col_number()
...
prepare_cronjob_data_nobindrows <- function(mypath = "~/cronjob/tableau-rls/",
                                            ...
                                            csv_date_end=-12,
                                            delim = ","){
  z <- import_unique_csvs_from_cronjob(mypath = mypath,
                                  skip = skip,
                                  n_max = n_max,
                                  locale = locale,
