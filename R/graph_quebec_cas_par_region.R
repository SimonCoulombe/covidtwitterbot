...
graph_pops <- map_int(levels(cases2$health_region),
                      ~populations %>%   filter(health_region == .x) %>% pull(pop) %>% as.integer())

cases_split <- cases2 %>% mutate(var_titre = health_region) %>%
  split(.$var_titre)
